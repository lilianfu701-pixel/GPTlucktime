...
export const entitlementConfigurations = pgTable("entitlement_configurations", {
  ...
  check("entitlement_configurations_quota_check", sql`${table.quotaLimit} IS NULL OR ${table.quotaLimit} >= 0`),
  check("entitlement_configurations_numeric_check", sql`${table.numericValue} IS NULL OR ${table.numericValue} >= 0`),
  ...
  check("entitlement_configurations_value_shape_check", sql`
    (${table.scope} = 'global_flag' AND ${table.booleanValue} IS NULL AND ${table.quotaLimit} IS NULL AND ${table.numericValue} IS NULL)
...
export const entitlementUserOverrides = pgTable("entitlement_user_overrides", {
  ...
  entitlementKey: varchar("entitlement_key", { length: 80 }).notNull()
    .references(() => entitlementDefinitions.key, { onDelete: "restrict" }),
  version: integer("version").notNull(),
  kind: text("kind").default("quota").notNull(),
  enabled: boolean("enabled").default(true).notNull(),
  booleanValue: boolean("boolean_value"),
  quotaLimit: integer("quota_limit"),
  numericValue: doublePrecision("numeric_value"),
  ...
  check("entitlement_user_overrides_quota_check", sql`${table.quotaLimit} IS NULL OR ${table.quotaLimit} >= 0`),
  check("entitlement_user_overrides_numeric_check", sql`${table.numericValue} IS NULL OR ${table.numericValue} >= 0`),
...
export const entitlementPlanBenefits = pgTable("entitlement_plan_benefits", {
  ...
  check("entitlement_plan_benefits_quota_check", sql`${table.quotaLimit} IS NULL OR ${table.quotaLimit} >= 0`),
  check("entitlement_plan_benefits_numeric_check", sql`${table.numericValue} IS NULL OR ${table.numericValue} >= 0`),
...
export const entitlementUsageCounters = pgTable("entitlement_usage_counters", {
  ...
  check("entitlement_usage_counters_used_check", sql`${table.used} >= 0`),
...
export const entitlementUsageOperations = pgTable("entitlement_usage_operations", {
  ...
  check("entitlement_usage_operations_amount_check", sql`${table.amount} > 0`),
  check("entitlement_usage_operations_kind_check", sql`${table.kind} IN ('boolean', 'quota', 'numeric')`),
  check("entitlement_usage_operations_limit_check", sql`${table.limit} IS NULL OR ${table.limit} >= 0`),
  check("entitlement_usage_operations_remaining_check", sql`${table.remaining} IS NULL OR ${table.remaining} >= 0`),
  check("entitlement_usage_operations_reason_check", sql`${table.reason} IS NULL OR ${table.reason} IN ('SAFETY_RESTRICTED', 'VERIFICATION_REQUIRED', 'FEATURE_DISABLED', 'NOT_INCLUDED', 'LIMIT_REACHED')`),
]);
