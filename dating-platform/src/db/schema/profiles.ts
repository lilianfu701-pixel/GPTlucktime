...
export const profiles = pgTable(
  ...
  (table) => [
    ...
    check("profiles_country_code_length_check", sql`char_length(${table.countryCode}) = 2`),
  ],
);
...
export const profilePreferences = pgTable(
  "profile_preferences",
  {
    ...
    countryCodes: text("country_codes").array().default(emptyTextArray).notNull(),
...
export const profilePhotos = pgTable(
  ...
  (table) => [
    ...
    index("profile_photos_profile_position_idx").on(table.profileId, table.position),
