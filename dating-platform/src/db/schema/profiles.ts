...
export const profilePhotoUploads = pgTable(
  "profile_photo_uploads",
  {
    ...
    finalizationStatus: text("finalization_status").default("pending").notNull(),
    finalizationLeaseId: uuid("finalization_lease_id"),
    finalizationLeaseExpiresAt: timestamptz("finalization_lease_expires_at"),
    stagingCleanupDueAt: timestamptz("staging_cleanup_due_at"),
    stagingDeletedAt: timestamptz("staging_deleted_at"),
    finalOrphanCleanupDueAt: timestamptz("final_orphan_cleanup_due_at"),
    finalOrphanDeletedAt: timestamptz("final_orphan_deleted_at"),
    artifactCleanupLeaseId: uuid("artifact_cleanup_lease_id"),
    artifactCleanupLeaseExpiresAt: timestamptz("artifact_cleanup_lease_expires_at"),
    createdAt: createdAt(),
    updatedAt: updatedAt(),
  },
  (table) => [
    ...
    index("profile_photo_uploads_finalize_claim_idx").on(table.finalizationStatus, table.finalizationLeaseExpiresAt),
    index("profile_photo_uploads_artifact_cleanup_idx").on(table.stagingCleanupDueAt, table.finalOrphanCleanupDueAt),
    index("profile_photo_uploads_artifact_cleanup_lease_idx").on(table.artifactCleanupLeaseExpiresAt),
    ...
    check(
      "profile_photo_uploads_finalization_status_check",
      sql`${table.finalizationStatus} IN ('pending', 'processing', 'finalized', 'cleanup')`,
    ),
  ],
);
