...
const handler = async (request: Request) => {
  ...
  const secret = readEnv(process.env).NOTIFICATION_WORKER_CRON_SECRET;
  if (!secret) return failure("NOTIFICATION_WORKER_UNAVAILABLE", "errors.serviceUnavailable", 503, true, traceId);
  ...
  try {
    return Response.json(await runConfiguredNotificationWorkers(),
      { headers: { "cache-control": "private, no-store", "x-trace-id": traceId } });
  } catch {
    return failure("NOTIFICATION_WORKER_FAILED", "errors.internal", 500, true, traceId);
  }
};
