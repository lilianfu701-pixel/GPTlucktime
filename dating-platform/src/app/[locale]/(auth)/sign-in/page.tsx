...
export default async function SignInPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className="grid min-h-screen bg-[var(--datecn-cream)] text-[var(--datecn-ink)] lg:grid-cols-[.9fr_1.1fr]">
    ...
    <section className="flex items-center justify-center px-4 py-10 sm:px-8"><div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-[0_22px_70px_rgba(75,31,39,.12)] ring-1 ring-[var(--datecn-ring)] sm:p-10"><Link className="font-serif text-2xl font-bold text-[var(--datecn-wine)] lg:hidden" href={`/${routeLocale}`}>{brand("name")}</Link><h1 className="mt-6 text-4xl font-bold tracking-tight">{t("title")}</h1><p className="mt-3 leading-7 text-[var(--datecn-muted)]">{t("intro")}</p><SignInForm locale={routeLocale} freeTestMode={freeTestMode} /><div className="mt-4 text-center"><Link className="font-bold text-[var(--datecn-wine)] hover:underline" href={`/${routeLocale}/demo/discover`}>{auth("demo")} →</Link></div><p className="mt-7 text-center text-xs text-[var(--datecn-muted)] lg:hidden">{auth("trust")}</p></div></section>
  </main>;
}
